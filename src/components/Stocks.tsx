...
const Stocks = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (isValidResponse<MarketIndex[]>(indicesData)) {
          setMarketIndices(indicesData.data);
        }
        if (isValidResponse<TopMover[]>(moversData)) {
          setTopMovers(moversData.data);
        }
        if (isValidResponse<MarketChart>(chartData)) {
          setMarketChart(chartData.data);
        }
        if (isValidResponse<HeatmapItem[]>(heatmapData)) {
          setMarketHeatmap(heatmapData.data);
        }
      } catch (err) {
        console.error("Fetch error:", err);
      } finally {
  ...
  return (
    <div>
      ...
      {/* Market Heatmap (Optional - Uncomment if needed) */}
      {/* <section className="bg-gray-50 py-12">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-xl font-semibold mb-4">Market Heatmap (Live)</h2>
          <div className="bg-white p-6 h-[600px] rounded-xl flex items-center justify-center text-gray-400 shadow-sm">
